 ...
 */
 Persistency.prototype.load = function(processId, processName, done) {
    ...
    var connection = this._getConnection();
    connection.readStreamEventsBackward(streamId, 0, 1, false, true, null, this.options.server.credentials, function(completed) {
        ...
        if(completed.result == 1){ // NoStream error code
            ...
            done(null, completed.events[0]); // We only fetch last event, so first entry in array is fine.
        }
    });
};
 ...
 */
 Persistency.prototype._createStreamId = function(processId, processName){
    return processName + '__' + processId;
}