 ...
 * @return {BPMNLane}
 */
BPMNProcess.prototype.getLane = function(taskName) {
    // First namemap the id, then find the lane that contains the ID
    var id = this.processDefinition.nameMap[taskName];
    for(var i = 0; i < this.processDefinition.lanes.length; i++){
        if(this.processDefinition.lanes[i].flowNodeRefs.indexOf(id) !== -1){
            return this.processDefinition.lanes[i];
        }
    }
    return {};
};
