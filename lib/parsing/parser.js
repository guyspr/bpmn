...
var flowNodeRef = require("./flowNodeRef.js");
...
exports.parse = function(bpmnXML, errorQueue, mainProcessName, fileName) {
    ...
    parser.onopentag = function (node) {
        ...
        if (inBPMN2Namespace) {
            ...
            if (currentProcessDefinition) {
                ...
                }else if (lane.isLaneName(localName)){
                    currentProcessElement = lane.createBPMNLane(node);
                    currentProcessDefinition.addLane(currentProcessElement);
                }else if(flowNodeRef.isFlowNodeRefName(localName)){
                    currentProcessElement = flowNodeRef.createBPMNFlowNodeRef(node);
                    //currentProcessDefinition.lanes[currentProcessDefinition.lanes.length - 1].addFlowNodeRefs(currentProcessElement);
                }
            }
        }
    };
    ...
    parser.ontext = function (t) {
        if(currentProcessElement && currentProcessElement.type == "flowNodeRef" && t.trim() !== ""){
            currentProcessDefinition.lanes[currentProcessDefinition.lanes.length - 1].addFlowNodeRefs(t);
        }
    };
